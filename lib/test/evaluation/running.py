...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    def save_bb(file, data):
        ...
        l = [[0 for i in range(8)] for j in range(len(data))]
        # len1 = tracked_bb.shape[0]
        for i in range(len(data)):
            l[i][0], l[i][1] = tracked_bb[i][0], tracked_bb[i][1]
            l[i][2], l[i][3] = tracked_bb[i][0] + tracked_bb[i][2], tracked_bb[i][1]
            l[i][4], l[i][5] = tracked_bb[i][0] + tracked_bb[i][2], tracked_bb[i][1] + tracked_bb[i][3]
            l[i][6], l[i][7] = tracked_bb[i][0],tracked_bb[i][1] + tracked_bb[i][3]
        bb = np.array(l, dtype=int)
        np.savetxt(file, bb, delimiter='\t', fmt='%d')
        # tracked_bb = np.array(data).astype(int)
