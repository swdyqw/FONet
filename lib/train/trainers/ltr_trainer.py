...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        def put_dict_cuda(dic):
            """ put dict to cuda"""
            cuda_dict = {}
            data_attr_ls = ['template_images', 'template_anno', 'template_masks', 'search_images', 'search_anno', 'search_masks', 
                            'valid', 'template_att', 'search_att', 'label']
            for key in dic:
                if (key in data_attr_ls):
                    cuda_dict[key] = dic[key].to(self.device)
                else:
                    cuda_dict[key] = dic[key]
            return cuda_dict
        ...
        for i, data in enumerate(loader, 1):
            ...
            if self.move_data_to_gpu:
                if data['visible']:
                    # 使用pin_memeroy的时候
                    data['visible'] = put_dict_cuda(data['visible'])
                    data['infrared'] = put_dict_cuda(data['infrared'])
